...
var toString2, getPrototypeOf, iterator, toStringTag, kindOf, kindOfTest, typeOfTest, isArray, isUndefined, isArrayBuffer, isString, isFunction, isNumber4, isObject, isBoolean, isPlainObject, isEmptyObject, isDate, isFile, isBlob, isFileList, isStream, isFormData, isURLSearchParams, isReadableStream, isRequest, isResponse, isHeaders, trim, _global, isContextDefined, extend, stripBOM, inherits, toFlatObject, endsWith, toArray2, isTypedArray, forEachEntry, matchAll, isHTMLForm, toCamelCase, hasOwnProperty, isRegExp, reduceDescriptors, freezeMethods, toObjectSet, noop, toFiniteNumber, toJSONObject, isAsyncFn, isThenable, _setImmediate, asap, isIterable, utils_default;
...
var init_toFormData = __esm({
  "node_modules/axios/lib/helpers/toFormData.js"() {
    ...
    init_AxiosError();
    init_null();
    predicates = utils_default.toFlatObject(utils_default, {}, null, function filter4(prop2) {
...
// node_modules/unofficial-metacritic/dist/index.mjs
function request(props) {
  return new Promise((resolve, reject) => {
    axios_default(props).then((response) => {
...
var BASE_URL, getMoviesInfoReview, getMoviesCriticReviews;
var init_dist3 = __esm({
  "node_modules/unofficial-metacritic/dist/index.mjs"() {
    init_browser();
    init_axios2();
    BASE_URL = "https://www.metacritic.com";
    getMoviesInfoReview = async (id) => {
      const requestOpts = {
        url: id,
        method: "get"
      };
    ...
    getMoviesCriticReviews = async (id) => {
      const requestOpts = {
...
var require_background = __commonJS({
  "extension/background.js"() {
    init_dist3();
    var CACHE_TTL_MS = 14 * 24 * 60 * 60 * 1e3;
    var STORAGE_KEY_PREFIX = "mc:title:";
    ...
    async function setCache(normalizedTitle, data2) {
      return new Promise((resolve) => {
        const value = { ...data2, timestamp: Date.now() };
    ...
    function parseScoresFromMoviePage(html3) {
      let critic = null;
      let user = null;
    ...
    function extractFirstMovieLinkFromSearch(html3) {
      const linkMatch = html3.match(/href=\"(\/movie\/[^\"?#]+)\"/i);
    ...
    async function tryFetchMoviePageBySlug(slug) {
      ...
      for (const url of urls) {
        try {
          const html3 = await fetchText(url);
          const { critic, user } = parseScoresFromMoviePage(html3);
    ...
    async function searchAndFetch(title) {
      ...
      for (const url of searchUrls) {
        try {
          const html3 = await fetchText(url);
          const link = extractFirstMovieLinkFromSearch(html3);
          if (link) {
    ...
    chrome.runtime.onMessage.addListener((message, _sender, sendResponse) => {
      if (message && message.type === "GET_METACRITIC_SCORES") {
        const { title } = message;
        getScoresForTitle(title).then((res) => sendResponse({ ok: true, data: res })).catch((err) => sendResponse({ ok: false, error: String(err) }));
        return true;
      }
      return false;
    });
